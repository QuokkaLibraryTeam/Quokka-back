...
from schemas.share import ShareCreate, ShareOut, TagsUpdate, ShareFilter
...
# ── C ────────────────────────────────────────────────
def publish(db: Session, *, story_id: int, user_id: uuid.UUID, data: ShareCreate) -> ShareOut:
    # 소유권 확인
    if not story_crud.get_one_filtered(db, filters=ShareFilter(story_id=story_id, user_id=user_id)):
        raise PermissionError("Story not found or access denied")

    tags = _clean(data.tags)

    existing = share_crud.get_one_filtered(db, filters=ShareFilter(story_id=story_id, user_id=user_id))
...
# ── U (PATCH) ────────────────────────────────────────
def patch_tags(db: Session, *, story_id: int, user_id: uuid.UUID, data: TagsUpdate) -> ShareOut:
    share = share_crud.get_one_filtered(db, filters=ShareFilter(story_id=story_id, user_id=user_id))
    if not share:
...
# ── D ────────────────────────────────────────────────
def unpublish(db: Session, *, story_id: int, user_id: uuid.UUID):
    share = share_crud.get_one_filtered(db, filters=ShareFilter(story_id=story_id, user_id=user_id))
    if not share:
        raise PermissionError("Shared story not found or access denied")
...
def is_shared(db: Session, story_id: int) -> bool:
    return share_crud.get_one_filtered(db, filters=ShareFilter(story_id=story_id)) is not None
